...
Bool LinkListInsert(LinkList *list, int i, int data)		//O(n)
{
...
Bool LinkListDelete(LinkList *list, int i, int* data)	//O(n)
{
...
Bool LinkListSet(LinkList *list, int i, int data)	//O(n)
{
	return ((i >= 0) && (i < LinkListLength(list))) ? (LinkListMove(list, i)->data = data, true) : false;
}

Bool LinkListGet(LinkList *list, int i, int* data)	//O(n)
{
...
{
	return ((list != NULL) && (list->m_current != NULL)) ? list->m_current->data : (int)0;
}

int LinkListFind(LinkList *list, int data)	//O(n)
{
	int ret = -1;		
